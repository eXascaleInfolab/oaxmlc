...
class Taxonomy:
    ...
    def load_taxonomy(self, cfg):
        ...
        with open(self.cfg['paths']['dataset'] / 'ontology.json') as f:
            for idx, line in enumerate(f):
                ...
                self.all_labels.add(data['label'])

        with open(self.cfg['paths']['dataset'] / 'taxonomy.txt') as f:
            for idx, line in enumerate(f):
                line = line.strip().replace('\n', '')
                parent, *children = line.split(' ')
                filtered_children = list(self.all_labels.intersection(children))
    ...
    def update_taxo(self, verbose=False):
        ...
        self.label_to_idx = {'root': 0}
        self.idx_to_label = {0: 'root'}
        # for idx, label in enumerate(self.all_children('root')):
    ...
    def print_stats(self):
        ...
        # Number of descriptions (-1 since we do not count the root)
    ...
    def print_graph_stats(self):
        ...
        print(f">> Average degree -> {np.mean([deg for _, deg in taxo_digraph.degree()])}")
    ...
    def merge_nodes(self, label_kept, label_deleted):
        ...
        # Remove from label2level and level2label
    ...
    def remove_leaves(self, leaves):
        ...
        for leaf in leaves:
            ...
            # Delete label2children and update label2parent
    ...
    def get_subtaxonomy(self, new_root_label):
        ...
        subtaxonomy.update_taxo(verbose=True)
    ...
    # Extract the parent-child relation into a file
