...
class CustomXMLHolder():
    def __init__(self, text_batch, task_id, beam_parameter, hector ,proba_operator = PROBA_COMBINE ):
        """
        ...
        self.hectree  : HecTree = self.hector.hector_forest[task_id]
    ...
    def run_predictions(self, batch_size = 15):
        ...
        while self.level +1 < self.hectree._max_level_tree :
            ...
            for i in range(0, len(txt),batch_size):
                ...
                out = self.hector.test_batch(documents_tokens=ctxt, paths=cpath,lvl_mask=clvl_mask,id_task=id_task).detach().cpu().numpy()
...
class CustomXMLHolderGlobal():
    def __init__(self, text_batch, task_id_list, beam_parameter, hector ,proba_operator = PROBA_COMBINE ):
        """
        ...
        self.hecforest  : list[HecTree] = self.hector.hector_forest

        self.n_tasks = len(self.hecforest)
        ...
        for i in self.ind :
            for j in self.task_id_list[i] :
                ...
                self.paths[j].append( [self.hecforest[j].root])
                self.prior[j].append(0.)

       
        self.level = 0  # start at level one

        self.proba = {ind: -1e9 * np.ones_like(self.hecforest[0]._mask_template) for ind in self.ind}  # will store logproba for each label
    ...
    def _get_data(self,task_index):
        ...
        lvl_mask = [self.hecforest[task_index].get_children_mask(label=a[-1]) for a in path]
        ...
        for ii in range(len(predictions)):
            ...
            if self._prev_index is None:
                ...
                for npath, nprior in sorted(self._selected_paths, key=lambda x: -x[1])[:self.beam_param]:
                    ...
                    if predicted_property not in self.hecforest[id_task].children_dict : # it is a leaf, let's stop there
                        continue
                    else:
    ...
    def _finalize_prediction(self):
        for index_document in self.ind:
            for task in range(self.n_tasks):
                self.proba[index_document][self.hecforest[task].root] = -1e9 # hide roots and task roots
        for index_label in  range(len(self.hecforest[0]._mask_template)):
            self.proba[index_document][index_label] = max(self.proba[index_document][index_label], -1e8)
    ...
    def run_predictions(self, batch_size = 15):
        ...
        max_depths = [ t._max_level_tree for t in self.hecforest]
