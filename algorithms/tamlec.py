...
from Hector.prediction_handler import CustomXMLHolder, CustomXMLHolderGlobal
...
class TamlecExp:
    ...
    def final_evaluation(self, split, metrics_handler, save_pred):
        ...
        xml = CustomXMLHolderGlobal(text_batch=input_data, task_id_list=task_id_list, beam_parameter=10, hector=self.model, proba_operator="MAX_PROBA")
        ...
        for task_id, (batched_input, batched_labels, task_relevant_classes) in enumerate(tqdm(self.dataloaders[f"tasks_{split}"], leave=False)):
            ...
            # Compute predictions
            xml = CustomXMLHolder(text_batch=input_data, task_id=task_id, beam_parameter=10, hector=self.model, proba_operator="MAX_PROBA")
            predictions, scores = xml.run_predictions(batch_size=256)
