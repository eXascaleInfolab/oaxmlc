...
def get_model(cfg, embeddings=None):
    model = MatchBaseModel(cfg, embeddings)
    return model


class MatchBaseModel(nn.Module):
    def __init__(self, cfg, embeddings):
        super(MatchBaseModel, self).__init__()
        self.cfg = cfg
        ...
        self.tewp = BaseBertModel(hidden_size, n_layers, n_probes, n_aheads, intermediate_size, dropout, emb_init=embeddings)

        # Dummy initialization, this is updated afterwards
        self.embedding = None

    # Write custom forward method to meet this framework's requirements
    def forward(self, input_ids, compute_average=True):
        doc_emb = self.tewp(input_ids)
        if compute_average:
            doc_emb = torch.mean(doc_emb, dim=1)

        return {'document_embedding': doc_emb}
