# Adapted from https://github.com/yuzhimanhua/MATCH/blob/master/deepxml/match.py
import torch
...
def get_model(cfg, embeddings=None):
    model = MatchModel(cfg, embeddings)
    return model
...
class MatchModel(nn.Module):
    def __init__(self, cfg, embeddings):
        ...
        self.cfg = cfg
        # Parameters taken from https://github.com/yuzhimanhua/MATCH/blob/master/configure/models/MATCH-MAG.yaml
        # Number of transformer encoder layers
        self.n_layers = 3
        # Need minimum 4 layers for cascadexml
        if self.cfg['method'] == 'cascadexml': self.n_layers = 4
        # Number of CLS token added
        self.n_probes = 8
        # Number of attention heads
        self.n_aheads = 2
        # Size of intermediate fully connected layer in BERT
        self.intermediate_size = 400
        self.dropout = 0.1
        self.hidden_size = self.cfg['emb_dim']
        self.tewp = BaseBertModel(self.hidden_size, self.n_layers, self.n_probes, self.n_aheads, self.intermediate_size,  self.dropout, emb_init=embeddings)
        self.sigmoid = nn.Sigmoid()

    def init_plaincls(self, labels_num):
        self.labels_num = labels_num
        self.plaincls = PlainC(self.labels_num, self.hidden_size, self.n_probes).to(self.cfg['device'])

    def forward(self, input_ids, output_embeddings=False, output_layers=False):
        context_vectors = self.tewp(input_ids, output_embeddings, output_layers)
        if output_layers:
            return context_vectors
        if output_embeddings:
            context_vectors = torch.mean(context_vectors, dim=1)
            return context_vectors
...
class BaseBertModel(nn.Module):
    def __init__(self, hidden_size, n_layers, n_probes, n_aheads, intermediate_size, dropout, hidden_act="relu", src_max_len=500, 
                 padding_idx=0, vocab_size=None, emb_init=None, emb_trainable=True, bottleneck_dim=None):
        super(BaseBertModel, self).__init__()   
        # Standard deviation of initialized CLS tokens embeddings  
        ...
        # Parameter that is saved and restored in state_dict, but it is not trained by the optimizer
    ...
    def forward(self, raw_input_variables, output_embeddings=False, output_layers=False):
        ...
        encoded_layers = self.encoder(embedding_output, extended_attention_mask, output_all_encoded_layers=output_layers)
        if output_layers:
